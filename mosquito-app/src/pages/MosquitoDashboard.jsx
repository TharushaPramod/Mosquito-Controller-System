...
const MosquitoDashboard = () => {
  ...
  useEffect(() => {
    ...
    onValue(dbRef, (snapshot) => {
      ...
      if (data) {
        ...
        // Process and format Hourly Data for the chart and table
        if (data.hourly) {
          const formattedData = Object.keys(data.hourly).map(key => ({
            id: key,
            ...data.hourly[key]
          }));
          // Sort data chronologically by the hour ID
  ...
  // --- 2. ALERT CHECKING EFFECT ---
  // Logic to trigger notifications based on mosquito count thresholds
  useEffect(() => {
    if (!liveData || hourlyData.length === 0) return;

    // Extract current hour from live timestamp (e.g., "10:15" -> "10")
    const currentHourStr = liveData.timestamp.split(':')[0]; 

    // Find statistics matching the current hour from the hourly dataset
    const currentHourStats = hourlyData.find(item => item.id === currentHourStr);

    if (currentHourStats) {
      const aedesCount = currentHourStats.aedes;

      // Condition: Trigger alert if Aedes count >= 2 AND an alert hasn't been sent for this hour yet
      if (aedesCount >= 2 && lastAlertHour.current !== currentHourStr) {
        
        // Optional: Audio alert logic can be implemented here
        ...
        const newAlert = {
          ...
          message: `Critical Aedes count detected`,
          type: "critical"
        };

        // Update notification list and unread badge count
        setNotifications(prev => [newAlert, ...prev]);
        setUnreadCount(prev => prev + 1);
        
        // Lock the alert for the current hour to prevent notification spam
        lastAlertHour.current = currentHourStr;
        
        console.log("🚨 ALERT TRIGGERED: ", aedesCount); 
      }
    }
  }, [liveData, hourlyData]); 

  // Toggle notification dropdown and reset unread count when opened
  ...
  // Helper function to format time strings into a 1-hour range (e.g., "10:00 - 11:00")
          ...
          <div className="flex items-center gap-4">
            <div className="hidden px-4 py-2 text-sm font-bold text-gray-700 bg-white rounded shadow md:block">
              CURRENT PERIOD: <span className="text-blue-600">
                {getTimeRange(liveData?.timestamp)}
              </span>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm text-left whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-100">
                {hourlyData.map((row) => (
                  // Highlight row in light red if Aedes count meets danger threshold
                  <tr key={row.id} className={`transition-colors ${row.aedes >= 2 ? 'bg-red-50' : 'hover:bg-gray-50'}`}>
                    <td className="px-4 py-3 font-medium text-gray-900">{getTimeRange(row.time)}</td>
                    ...
                    {/* Visual warning for high Aedes counts */}
                    <td className={`px-4 py-3 text-center font-medium ${row.aedes >= 2 ? 'text-red-700 font-bold' : 'text-red-600'}`}>
                      {row.aedes}
                    </td>
